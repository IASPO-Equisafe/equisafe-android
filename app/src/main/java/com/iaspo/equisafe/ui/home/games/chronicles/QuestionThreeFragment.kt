...
import com.iaspo.equisafe.databinding.FragmentQuestionThreeBinding
...
class QuestionThreeFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        arguments?.let {
            ...
            val questionThree = questionList[2].question

            binding.questionThreeChronicles.text = questionThree
            answerQuestion = questionList[2].answer.lowercase(Locale.ROOT)
        }
    ...
    private fun setupAction() {
        binding.btnNextQuestionThree.setOnClickListener {
            val questionFourFragment = QuestionFourFragment.newInstance(listQuestionGame, correctAnswer)
            val fragmentManager = parentFragmentManager
            fragmentManager.beginTransaction().apply {
                replace(
                    R.id.frame_container,
                    questionFourFragment,
                    QuestionFourFragment::class.java.simpleName
                )
                addToBackStack(null)
                commit()
            }
        ...
        binding.btnCheckQuestionThree.setOnClickListener {
            val answerUser = binding.inputAnswerThree.text.toString().trim().lowercase(Locale.ROOT)

            if (answerUser == answerQuestion) {
                binding.btnCheckQuestionThree.gone()
    ...
    companion object {
        private const val EXTRA_QUESTION = "extra_questions"
        private const val EXTRA_RESULT = "extra_result"

        fun newInstance(questionList: ArrayList<QuestionGame>, correctAnswer: Int): QuestionThreeFragment {
            val fragment = QuestionThreeFragment()
            val args = Bundle()
